...
describe("API E2E - Email Flow", () => {
    ...
    async () => {
      ...
      const { box } = await client.box.create({
        ...
        ...(TEST_ENV.CLAUDE_CODE_OAUTH_TOKEN && {
          envVars: {
            CLAUDE_CODE_OAUTH_TOKEN: TEST_ENV.CLAUDE_CODE_OAUTH_TOKEN,
          },
        }),
      });
      boxId = box.id;
      ...
      const { box: deployedBox } = await waitFor(
        ...
        },
        {
          ...
          until: (r) => r.box.status === "running" || r.box.status === "error",
        }
      );

      if (deployedBox.status === "error") {
        throw new Error(`Box deployment failed: ${deployedBox.status}`);
      }

      console.log(`Box deployed: ${deployedBox.spriteUrl}`);
      console.log("=== Setup Complete ===\n");
    },
    6 * 60 * 1000
  ); // 6 minute timeout for setup
  ...
  test(
    ...
    async () => {
      ...
      // Wait for email to be delivered (check box emails via SDK)
      console.log("Waiting for email delivery...");
      const { emails } = await waitFor(
        async () => {
          const result = await client.boxDetails.emails({ id: boxId });
        ...
        },
        {
          ...
          until: (r) => r.emails.length > 0,
        }
      );

      expect(emails.length).toBeGreaterThan(0);
      const deliveredEmail = emails[0]!;
      expect(deliveredEmail.status).toBe("delivered");
      console.log(`Email delivered: ${deliveredEmail.id}`);

      // Wait for Claude session to be created (check sessions via SDK)
      console.log("Waiting for Claude session...");
      const { sessions } = await waitFor(
        async () => {
          const result = await client.boxDetails.sessions({ id: boxId });
          console.log(`Session count: ${result.sessions.length}`);
          return result;
        },
        {
          timeoutMs: 120_000, // 2 minutes for Claude to start
          pollIntervalMs: 5000,
          description: "Claude session creation",
          until: (r) => r.sessions.length > 0,
        }
      );
